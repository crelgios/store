...
const categoryDefaults = {
  "Pakistani Suits": {
    ...
    description: "A refined Pakistani suit set designed with a modern ethnic look, comfortable styling, and elegant detailing."
  }
};
...
export function generateProductDraft({ price, gender, category, color, sizes, stock, notes }) {
  ...
  const finalCategory = cleanAuto(category, "Pakistani Suits");
  const finalColor = cleanAuto(color, "Elegant");
  const finalSizes = parseList(sizes || "XS, S, M, L, XL, XXL");
  const defaultCopy = categoryDefaults[finalCategory] || categoryDefaults["Pakistani Suits"];
  ...
  const description = `${defaultCopy.description}${colorText} Suitable for customers looking for Pakistani suit styles with a clean, modern shopping experience.${extra}`;
  ...
  return {
    ...
    tags: ["pakistani suits", finalCategory.toLowerCase(), finalGender.toLowerCase()].filter(Boolean),
